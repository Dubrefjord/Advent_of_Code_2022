...
def main():
    ...
    for line in open('input15.txt'):
        ...
        intervals = get_no_beacon_intervals(sensor,dist,row_number).items()
...
def get_no_beacon_intervals(sensor,dist,row_number):
    x,y = sensor
    if abs(row_number-y) < dist:
        return {row_number:(x-dist+abs(row_number), x+dist-abs(row_number))}
    return {}
